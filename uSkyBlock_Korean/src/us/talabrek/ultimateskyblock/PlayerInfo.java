...
public class PlayerInfo implements Serializable {
	...
	public PlayerInfo(String playerName, boolean hasIsland, int iX, int iY,
			int iZ, int hX, int hY, int hZ) {
		this.playerName = playerName;
		this.hasIsland = hasIsland;
		if ((iX == 0) && (iY == 0) && (iZ == 0))
			this.islandLocation = null;
		else
			this.islandLocation = getStringLocation(new Location(
					uSkyBlock.getSkyBlockWorld(), iX, iY, iZ));
		if ((hX == 0) && (hY == 0) && (hZ == 0))
			this.homeLocation = null;
		else
			this.homeLocation = getStringLocation(new Location(
					uSkyBlock.getSkyBlockWorld(), hX, hY, hZ));
	...
	public void setLeaveParty() {
		...
		if (Bukkit.getPlayer(this.playerName) == null)
			getPlayerConfig(this.playerName).set("player.kickWarning",
			...
			if (uSkyBlock
					...
					.contains(
							"options.challenges.challengeList." + challenge
									+ ".resetInHours")) {
				((Challenge) this.challengeListNew.get(challenge))
						.setFirstCompleted(Calendar.getInstance()
								.getTimeInMillis()
								+ uSkyBlock
										.getInstance()
										.getConfig()
										.getInt("options.challenges.challengeList."
												+ challenge + ".resetInHours")
								* 3600000);
			} else if (uSkyBlock.getInstance().getConfig()
					.contains("options.challenges.defaultResetInHours")) {
				((Challenge) this.challengeListNew.get(challenge))
						.setFirstCompleted(Calendar.getInstance()
								.getTimeInMillis()
								+ uSkyBlock
										.getInstance()
										.getConfig()
										.getInt("options.challenges.defaultResetInHours")
								* 3600000);
			} else {
				((Challenge) this.challengeListNew.get(challenge))
						.setFirstCompleted(Calendar.getInstance()
								.getTimeInMillis() + 518400000L);
			}
		}
	...
	public long getChallengeCooldownTime(String challenge) {
		if (getChallenge(challenge).getFirstCompleted() > 0L) {
			if (getChallenge(challenge).getFirstCompleted() > Calendar
					.getInstance().getTimeInMillis()) {
				return getChallenge(challenge).getFirstCompleted()
	...
	public boolean onChallengeCooldown(String challenge) {
		if (getChallenge(challenge).getFirstCompleted() > 0L) {
			return getChallenge(challenge).getFirstCompleted() > Calendar
					.getInstance().getTimeInMillis();
		}
	...
	public void resetChallenge(String challenge) {
		...
		((Challenge) this.challengeListNew.get(challenge))
				.setFirstCompleted(0L);
	}

	public int checkChallenge(String challenge) {
		try {
			if (this.challengeListNew.containsKey(challenge.toLowerCase())) {
				return ((Challenge) this.challengeListNew.get(challenge
						.toLowerCase())).getTimesCompleted();
			}
	...
	public int checkChallengeSinceTimer(String challenge) {
		try {
			if (this.challengeListNew.containsKey(challenge.toLowerCase())) {
				return ((Challenge) this.challengeListNew.get(challenge
						.toLowerCase())).getTimesCompletedSinceTimer();
			}
		} catch (ClassCastException localClassCastException) {
		}
		return 0;
	...
	public void displayData(String player) {
		...
		if (getIslandLocation() != null)
			System.out.print(player + " island location: "
					+ getIslandLocation().toString());
		if (getHomeLocation() != null)
			System.out.print(player + " home location: "
					+ getHomeLocation().toString());
	}

	public void buildChallengeList() {
		if (this.challengeListNew == null)
			this.challengeListNew = new HashMap<String, Challenge>();
		Iterator<?> itr = Settings.challenges_challengeList.iterator();
		while (itr.hasNext()) {
			String current = (String) itr.next();
			if (this.challengeListNew.containsKey(current.toLowerCase()))
				continue;
			this.challengeListNew.put(current.toLowerCase(), new Challenge(
					current.toLowerCase(), 0L, 0, 0));
		}

		if (this.challengeListNew.size() <= Settings.challenges_challengeList
				.size())
			return;
		Object[] challengeArray = this.challengeListNew.keySet().toArray();
		for (int i = 0; i < challengeArray.length; i++) {
			if (Settings.challenges_challengeList.contains(challengeArray[i]
					.toString()))
				continue;
	...
	public void displayChallengeList() {
		...
		while (itr.hasNext()) {
			...
			System.out.print(current + ": "
					+ this.challengeListNew.get(current));
		}
	}
	...
	public void setupPlayer(String player) {
		...
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "hasIsland");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "islandX");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "islandY");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "islandZ");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "homeX");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "homeY");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "homeZ");
		getPlayerConfig(player);
		FileConfiguration.createPath(getPlayerConfig(player)
				.getConfigurationSection("player"), "challenges");
		...
		getPlayerConfig(player).set("player.kickWarning",
				Boolean.valueOf(false));
		Iterator<String> ent = this.challengeListNew.keySet().iterator();
		String currentChallenge = "";
		while (ent.hasNext()) {
			currentChallenge = (String) ent.next();
			getPlayerConfig(player).createSection(
					"player.challenges." + currentChallenge);
			getPlayerConfig(player);
			FileConfiguration.createPath(
					getPlayerConfig(player).getConfigurationSection(
							"player.challenges." + currentChallenge),
					"firstCompleted");
			getPlayerConfig(player);
			FileConfiguration.createPath(
			...
			FileConfiguration.createPath(
					getPlayerConfig(player).getConfigurationSection(
							"player.challenges." + currentChallenge),
					"timesCompletedSinceTimer");
			getPlayerConfig(player)
					.set("player.challenges." + currentChallenge
							+ ".firstCompleted",
							Long.valueOf(((Challenge) this.challengeListNew
									.get(currentChallenge)).getFirstCompleted()));
			getPlayerConfig(player)
					.set("player.challenges." + currentChallenge
							+ ".timesCompleted",
							Integer.valueOf(((Challenge) this.challengeListNew
									.get(currentChallenge)).getTimesCompleted()));
			getPlayerConfig(player).set(
					"player.challenges." + currentChallenge
							+ ".timesCompletedSinceTimer",
					Integer.valueOf(((Challenge) this.challengeListNew
							.get(currentChallenge))
							.getTimesCompletedSinceTimer()));
		}
	}
	...
	public PlayerInfo loadPlayer(String player) {
		...
		try {
			this.hasIsland = getPlayerConfig(player).getBoolean(
					"player.hasIsland");
			this.islandLocation = getStringLocation(new Location(
					uSkyBlock.getSkyBlockWorld(), getPlayerConfig(player)
							.getInt("player.islandX"), getPlayerConfig(player)
							.getInt("player.islandY"), getPlayerConfig(player)
							.getInt("player.islandZ")));
			this.homeLocation = getStringLocation(new Location(
					uSkyBlock.getSkyBlockWorld(), getPlayerConfig(player)
							.getInt("player.homeX"), getPlayerConfig(player)
							.getInt("player.homeY"), getPlayerConfig(player)
							.getInt("player.homeZ")));
			buildChallengeList();

			Iterator<?> ent = Settings.challenges_challengeList.iterator();
			String currentChallenge = "";
			this.challengeListNew = new HashMap<String, Challenge>();
			while (ent.hasNext()) {
				currentChallenge = (String) ent.next();
				this.challengeListNew
						.put(currentChallenge,
								new Challenge(
										currentChallenge,
										getPlayerConfig(player).getLong(
												"player.challenges."
														+ currentChallenge
														+ ".firstCompleted"),
										getPlayerConfig(player).getInt(
												"player.challenges."
														+ currentChallenge
														+ ".timesCompleted"),
										getPlayerConfig(player)
												.getInt("player.challenges."
														+ currentChallenge
														+ ".timesCompletedSinceTimer")));
			}
			...
			if ((Bukkit.getPlayer(player) != null)
					&& (getPlayerConfig(player)
							.getBoolean("player.kickWarning"))) {
				Bukkit.getPlayer(player)
						.sendMessage(
								"§cYou were removed from your island since the last time you played!");
				getPlayerConfig(player).set("player.kickWarning",
						Boolean.valueOf(false));
			}
	...
	public void reloadPlayerConfig(String player) {
		this.playerConfigFile = new File(
				uSkyBlock.getInstance().directoryPlayers, player + ".yml");
	...
	public void savePlayerConfig(String player) {
		...
		getPlayerConfig(player).set("player.hasIsland",
				Boolean.valueOf(getHasIsland()));
		if (getIslandLocation() != null) {
			getPlayerConfig(player).set("player.islandX",
					Integer.valueOf(getIslandLocation().getBlockX()));
			getPlayerConfig(player).set("player.islandY",
					Integer.valueOf(getIslandLocation().getBlockY()));
			getPlayerConfig(player).set("player.islandZ",
					Integer.valueOf(getIslandLocation().getBlockZ()));
		} else {
			getPlayerConfig(player).set("player.islandX", Integer.valueOf(0));
			getPlayerConfig(player).set("player.islandY", Integer.valueOf(0));
			getPlayerConfig(player).set("player.islandZ", Integer.valueOf(0));
		...
		if (getHomeLocation() != null) {
			getPlayerConfig(player).set("player.homeX",
					Integer.valueOf(getHomeLocation().getBlockX()));
			getPlayerConfig(player).set("player.homeY",
					Integer.valueOf(getHomeLocation().getBlockY()));
			getPlayerConfig(player).set("player.homeZ",
					Integer.valueOf(getHomeLocation().getBlockZ()));
		} else {
			getPlayerConfig(player).set("player.homeX", Integer.valueOf(0));
			getPlayerConfig(player).set("player.homeY", Integer.valueOf(0));
			getPlayerConfig(player).set("player.homeZ", Integer.valueOf(0));
		...
		try {
			...
			uSkyBlock
					...
					.getLogger()
					.log(Level.SEVERE,
							"Could not save config to " + this.playerConfigFile,
							ex);
		}
	}
